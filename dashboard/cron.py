...
class DashboardCronJob(CronJobBase):
    ...
    def update_with_bq_access(self):
        ...
        for DATA_WAREHOUSE_course_id in split_list(Course.objects.get_supported_courses(), settings.CRON_BQ_IN_LIMIT):
            ...
            query = """select CAST(SUBSTR(JSON_EXTRACT_SCALAR(event, '$.object.id'), 35) AS STRING) AS file_id,
                    ...
                    """
            logger.debug(query)
            logger.debug(DATA_WAREHOUSE_course_id)
            query_params = [
                bigquery.ArrayQueryParameter('course_ids', 'STRING', DATA_WAREHOUSE_course_id),
            ]
            ...
            return_string += str(df.shape[0]) + " rows for courses " + ",".join(DATA_WAREHOUSE_course_id) + "\n"
